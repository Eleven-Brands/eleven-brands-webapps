...
def build_st_dataframe(all_orders_df_analysis: pd.DataFrame, group_by_cols: list):
    """
    ...
    event_sales = _build_event_sales_df(
        all_orders_df_analysis, target_sales, committed, group_by_cols
    )

    # Calculate Differences
    event_sales["units_diff"] = event_sales["units_sold"] - event_sales["target_units"]
    event_sales["committed_diff"] = (
        event_sales["units_sold"] - event_sales["committed_units"]
    )
...
def _build_event_sales_df(
    ...
    actual = (
        actual_sales.groupby(group_by_cols)
        .agg(units_sold=("quantity", "sum"), revenue=("item_price", "sum"))
        .reset_index()
    )

    target = (
        target_sales.groupby(group_by_cols)
        .agg(
    ...
    committed = (
        committed_units.groupby(group_by_cols)
        .agg(committed_units=("committed_units", "sum"))
        .reset_index()
    )

    merged = (
        skus_combined.merge(actual, on=group_by_cols, how="left")
