...
def hourly_chart(
     *,
     ...
     st_container.altair_chart(
          _build_altair(df_long, actual_col, forecast_col, target_col, is_percent, has_display_labels),
    ...
    *,
    ...
    target_col: str,
     ...
     df_grouped = (
          ...
          .agg(actual=(actual_col, "sum"),forecast=(forecast_col, "sum"), target=(target_col, "sum"))
     )
     ...
     forecast_bars = (
          ...
          .mark_bar(cornerRadiusTopLeft=5, cornerRadiusTopRight=5, opacity=0.7, size=40, color="#ea4335")
          .encode(
               x=x,
               y=alt.Y("forecast:Q", title="Units Sold",),
               tooltip=[date_col, alt.Tooltip("forecast:Q", title="Forecast", format=".1f")]
          )
     )
     ...
     actual_bars = (
          alt.Chart(df_grouped)
          .mark_bar(cornerRadiusTopLeft=5, cornerRadiusTopRight=5, size=20, color="#98b9d0")
          .encode(
               x=x,
               y=alt.Y("actual:Q"),
     ...
     target_bars = (
          alt.Chart(df_grouped)
          .mark_bar(cornerRadiusTopLeft=5, cornerRadiusTopRight=5, opacity=0.3, size=60, color= "#d89663")
          .encode(
               x=x,
     ...
     chart = (target_bars + forecast_bars + actual_bars).properties(
          width="container", height=300, title="Actual vs Forecast vs Target (in order thinner to thicker)"
     )
...
def _df_for_plot(df, actual_col, forecast_col, target_col):
     ...
     return df[["sales_region", "local_date", "local_hour"] + [actual_col, forecast_col, target_col]]
...
def _build_altair(
     ...
     base = alt.Chart(df).encode(
          ...
          color=alt.Color(
               "type:N",
               title="Series",
               scale=alt.Scale(domain=[actual_label, forecast_label, target_label], range=[ "#98b9d0", "#ea4335",  "#d89663" ]),
     ...
     # Point & Labels
     two_series = (
          (alt.datum.type == actual_label)
          | (alt.datum.type == forecast_label)
     )

     points = (
          base
               .mark_point(size=60, opacity=0.7)
               .transform_filter(two_series)
)

     labels_above = (
          base
          .mark_text(dy=-25, fontSize=15)
