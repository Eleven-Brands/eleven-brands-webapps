...
@st.cache_data
def compute_hourly_share_profile(
     event_name: str
) -> pd.DataFrame:
     """
     ...
     base_profile = (
          long_shares
          .groupby(["sales_region", "event_day", "local_hour"], as_index=False)
     ...
     profile = (
          ...
          .groupby(
               ["sales_region", "new_event_day", "local_hour"],
               as_index=False
          )
...
def _get_past_hourly_sales_for_event(
     event_name: str
) -> pd.DataFrame:
     ...
     df = (
          ...
          .groupby( ["event_date", "local_hour", "sales_region", "event_name", "event_day"], as_index=False )
