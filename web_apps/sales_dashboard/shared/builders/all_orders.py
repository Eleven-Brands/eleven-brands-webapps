...
def build_all_orders_actual_event(
    all_orders_df: pd.DataFrame, regions: List[str], event_dates: List[datetime.date]
...
def _get_all_orders_with_product_model_df() -> pd.DataFrame:
    ...
    merged = (
        all_orders.merge(sales_channel, on="sales_channel", how="inner")
...
def _get_all_orders_with_local_datetime_df() -> pd.DataFrame:
    ...
    df["__datetime_utc"] = pd.to_datetime(
...
def _filter_orders_by_regions_and_dates(
    df: pd.DataFrame, regions: List[str], dates: List[datetime.date]
) -> pd.DataFrame:
    """
...
def _fill_region_date_hour_grid(
    agg_df: pd.DataFrame, regions: List[str], dates: List[datetime.date]
) -> pd.DataFrame:
    """
    ...
    full_index = pd.MultiIndex.from_product(
        [regions, dates, hours], names=["sales_region", "local_date", "local_hour"]
    )

    df = (
        agg_df.set_index(["sales_region", "local_date", "local_hour"])
...
def _add_daily_cumulative(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["cum_quantity"] = df.groupby(["sales_region", "local_date"])["quantity"].cumsum()
...
def _get_last_actual_thresholds_by_region(agg_df: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        agg_df.groupby("sales_region", as_index=False)
