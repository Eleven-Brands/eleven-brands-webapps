...
for col, label in zip(cols, sales_labels):
    ...
    clicked = col.button(
        label, key=f"btn_{label}", disabled=is_sel, use_container_width=True
    )
    if clicked:
...
hourly_totals_all_orders_df_analysis = all_orders_df_analysis.groupby(
    ["local_date", "local_hour"], as_index=False
).agg(quantity=("quantity", "sum"), item_price=("item_price", "sum"))

last_event_hourly_totals_all_orders_df_analysis = (
    last_event_all_orders_analysis.groupby(
        ["local_date", "local_hour"], as_index=False
    ).agg(quantity=("quantity", "sum"), item_price=("item_price", "sum"))
)
...
layout = [
    ...
    (
        "Units Sold by Hour",
        "avg_units_sold",
        f"{metrics['diff_units_vs_last']} vs last Prime Day",
    ),
    (
        "Revenue by Hour",
        "avg_revenue",
        f"{metrics['diff_revenue_vs_last']} vs last Prime Day",
    ),
]
...
amazon_family_totals_all_orders_df_analysis = all_orders_df_analysis.groupby(
    ["amazon_family"], as_index=False
).agg(quantity=("quantity", "sum"), item_price=("item_price", "sum"))
...
daily_totals_all_orders_df_analysis = all_orders_df_analysis.groupby(
    ["local_date"], as_index=False
).agg(quantity=("quantity", "sum"), item_price=("item_price", "sum"))

daily_totals_all_orders_df_analysis["local_date"] = pd.to_datetime(
    daily_totals_all_orders_df_analysis["local_date"]
)
daily_totals_all_orders_df_analysis["date_label"] = daily_totals_all_orders_df_analysis[
    "local_date"
].dt.strftime("%b %d")
...
(
    ...
    col_units_operation,
    col_units_value,
    col_revenue_operation,
    col_revenue_value,
) = st.columns([4, 4, 2, 4, 2, 4, 2])
...
event_sales_by_amazon_family = build_st_dataframe(
    all_orders_df_analysis, [c for c in group_by_cols if c != "sku"]
)
...
event_sales_by_amazon_family = event_sales_by_amazon_family[
    [c for c in cols if c != "sku"]
]
...
with col_sku:
    search_sku = st.text_input("Search SKU", placeholder="Type SKU or Base SKU...")

with col_family:
    search_amazon_family = st.multiselect(
        "Amazon Family", options=all_orders_df_analysis["amazon_family"].unique()
    )
...
with col_units_value:
    ...
    if u_op == "Between":
        u_lo, u_hi = st.slider(
            "Units % Range", 0.0, u_max, (0.0, u_max), step=1.0, format="%.0f"
        )
    else:
...
with col_revenue_value:
    ...
    if r_op == "Between":
        r_lo, r_hi = st.slider(
            "Revenue % range", 0.0, r_max, (0.0, r_max), step=1.0, format="%.0f"
        )
    else:
...
if u_op == "Greater than or equal to":
    ...
    df = df[(df["progress_units_sold"] >= u_lo) & (df["progress_units_sold"] <= u_hi)]
...
if r_op == "Greater than or equal to":
    ...
    df = df[(df["progress_revenue"] >= r_lo) & (df["progress_revenue"] <= r_hi)]
...
st.dataframe(
    ...
    column_config={
        ...
        "event_date": st.column_config.DateColumn(
            "Event Date", format="YYYY-MM-DD", pinned=True
        ),
        # Units Columns
        ...
        "progress_units_sold": st.column_config.ProgressColumn(
            "Units Sold / Target", format="%.0f%%", min_value=0.0, max_value=100
        ),
        "committed_units": st.column_config.NumberColumn(
            "Committed Units", format="%.0f"
        ),
        "committed_diff": st.column_config.NumberColumn(
            "Δ Committed Units", format="%.0f"
        ),
        "progress_committed": st.column_config.ProgressColumn(
            "Units Sold / Committed Units",
            format="%.0f%%",
            min_value=0.0,
            max_value=100,
        ),
...
def test():
    ...
    event_sales = build_st_dataframe(
        all_orders_df_analysis, ["sales_region", "amazon_family", "sku", "event_date"]
    )
